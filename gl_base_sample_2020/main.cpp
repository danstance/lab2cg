...
Camera camera(float3(-30.0f, 25.0f, 0.0f));
...
int main(int argc, char** argv)
{
    ...
    GLsizei AirShipConus = CreateConus(1.0f, 1.0f, 5000, vaoAirShipConus);
    GLsizei AirShipCabin = CreateCuboid(1.0f, 1.0f, 1.0f, vaoAirShipCabin);
    ////////////////////////////////////////////////////////////////////////////////
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            model = transpose(mul(translate4x4(float3(-1.0f , 2.0f , 14.0f )), scale4x4(float3(1.0f, 1.0f, 1.0f))));
        ...
        {
            model = transpose(mul(translate4x4(float3(-3.5f * sin(timer * 0.1), 2.0f * cos(timer * 0.1), 15.0f * tan(timer * 0.1))), scale4x4(float3(2.5f, 4.5f, 2.5f))));
            lambert.SetUniform("model", model); GL_CHECK_ERRORS;
        ...
        {
            model = transpose(mul(translate4x4(float3(1.5f*sin(timer*0.1), 2.0f * cos(timer * 0.1), 14.0f * tan(timer * 0.1))), scale4x4(float3(1.0f, 0.5f, 1.0f))));
            lambert.SetUniform("model", model); GL_CHECK_ERRORS;
        ...
        {
            model = transpose(mul(translate4x4(float3(3.2f*sin(timer*0.1), 2.0f*cos(timer*0.1), 14.0f*tan(timer*0.1))), scale4x4(float3(0.6f, 1.5f, 0.6f))));
            lambert.SetUniform("model", model); GL_CHECK_ERRORS;
        ...
        glBindVertexArray(vaoForest);
        {
            for (int i = 1; i < 700; i++)
            {
                float4x4 model1 = transpose(mul(translate4x4(float3(cos(20 * i) * 19.0f, -5.99f, cos(i) * 19.0f)), scale4x4(float3(0.5f, (0.7 + sin(i / 2) / 3 * pow((-1), i)) * 1.0f, 0.5f))));
        ...
        {
            float4x4 modelship = transpose(mul(rotate_Y_4x4(timer*0.05),mul(translate4x4(float3(0.0f, 7.0f, -15.0f)), scale4x4(float3(5.55f, 2.55f, 2.55f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipBody, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

             modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-10.0f , 15.0f, -15.0f)), scale4x4(float3(5.55f, 2.55f, 2.55f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipBody, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;


            modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-10.0f, -1.0f, -15.0f)), scale4x4(float3(5.55f, 2.55f, 2.55f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipBody, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;


             modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(0.0f, 7.0f, 15.0f)), scale4x4(float3(5.55f, 2.55f, 2.55f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipBody, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
        ...
        {
            float4x4 modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(4.0f, 7.0f , -15.0f)), scale4x4(float3(1.55f, 3.55f, 0.10f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
             modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(4.0f, 6.25f , -15.0f)), scale4x4(float3(0.15f, 2.55f, 4.55f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

             modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-6.0f, 15.0f , -15.0f)), scale4x4(float3(1.55f, 3.55f, 0.10f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
            modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-6.0f, 14.25f, -15.0f)), scale4x4(float3(0.15f, 2.55f, 4.55f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

            modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-6.0f, -1.0f , -15.0f)), scale4x4(float3(1.55f, 3.55f, 0.10f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
            modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-6.0f, -2.25f , -15.0f)), scale4x4(float3(0.15f, 2.55f, 4.55f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

             modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-4.0f, 7.0f, 15.0f)), scale4x4(float3(1.55f, 3.55f, 0.10f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
            modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-4.0f, 6.25f, 15.0f)), scale4x4(float3(0.15f, 2.55f, 4.55f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            ...
            modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-14.0f, 15.0f, 15.0f)), scale4x4(float3(1.55f, 3.55f, 0.10f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
            modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-14.0f, 14.25f, 15.0f)), scale4x4(float3(0.15f, 2.55f, 4.55f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

            modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-14.0f, -1.0f, 15.0f)), scale4x4(float3(1.55f, 3.55f, 0.10f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
            modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-14.0f, -2.25f, 15.0f)), scale4x4(float3(0.15f, 2.55f, 4.55f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
        ...
        ///////////////////    Кабина коробля
        glBindVertexArray(vaoAirShipCabin);
        {
            float4x4 modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-1.5f, 4.0f, -15.0f)), scale4x4(float3(2.55f, 1.55f, 1.10f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipCabin, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

            modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-11.5f, 12.0f, -15.0f)), scale4x4(float3(2.55f, 1.55f, 1.10f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipCabin, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

            modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-11.5f, -4.0f , -15.0f)), scale4x4(float3(2.55f, 1.55f, 1.10f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipCabin, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

            modelship = transpose(mul(rotate_Y_4x4(timer * 0.05), mul(translate4x4(float3(-1.5f, 4.0f, 15.0f)), scale4x4(float3(2.55f, 1.55f, 1.10f)))));
            lambert.SetUniform("model", modelship); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, AirShipCabin, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
